...
{
    /**
     * @return Collection<string, \Illuminate\Database\Eloquent\Collection<int, Elephpant>>
     */
    public function fetchAllOrderedAndGrouped(): Collection
    {
        ...
        return Elephpant::query()
            ->select()
            ->addSelect([
                'possible_receivers' => function ($query) use ($loggedUserId): void {
                    $this->getPossibleReceivers($query, $loggedUserId);
                },
                'possible_senders' => function ($query) use ($loggedUserId): void {
    ...
    public function getPossibleSenders($query, $authUserId): void
    {
        $query->select([Elephpant::raw('group_concat(u.username) as nb_users')])
            ...
            ->whereExists(function ($query2) use ($authUserId): void {
                ...
                $query2->select('elephpant_id')
                    ...
                    ->whereNotExists(function ($query3): void {
                        // That
    ...
    public function getPossibleReceivers($query, $authUserId): void
    {
        $query->select([Elephpant::raw('group_concat(distinct u.id) as nb_users')])
            ->from('users as u')
            ->whereNotIn('u.id', [$authUserId])
            ->whereNotExists(function ($query2): void {
            ...
            ->whereExists(function ($query3) use ($authUserId): void {
            ...
            ->whereExists(function ($query4) use ($authUserId): void {
                ...
                $query4
                    ...
                    ->whereNotExists(function ($query5) use ($authUserId): void {
                        $query5
