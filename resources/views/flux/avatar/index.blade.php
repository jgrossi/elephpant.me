...
<flux:with-tooltip :$tooltip :$attributes>
    <flux:button-or-link :attributes="$attributes->class($classes)->merge($circle ? ['data-circle' => 'true'] : [])" :$as :$href data-flux-avatar data-slot="avatar" data-size="{{ $size }}">
        <?php if ($src): ?>
            {{-- Initials stay underneath so a failed remote image can fall back without a broken icon. --}}
            <?php if ($hasTextContent && ! $icon): ?>
                <span class="select-none">{{ $initials ?? $slot }}</span>
            <?php endif; ?>
            <img src="{{ $src }}" alt="{{ $alt ?? $name }}" class="absolute inset-0 rounded-[var(--avatar-radius)] size-full object-cover" onerror="this.remove()">
        <?php elseif ($icon): ?>
